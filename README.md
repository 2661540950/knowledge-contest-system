...
- 「取消」= 丢弃旧进度，从第 1 题重新开始
- 首页会显示一行「📌 上次练习模式进度：…」提示；一轮做完进入结果页后自动清除

背题模式是浏览模式，不记录进度；错题练习是临时集合，也不记录。

## 界面

- 页面顶部没有大标题，答题 / 背题时最上方是一条 **sticky 进度条**（第 x / N 题 + 得分 / 背题模式 + 主页按钮），滚动时始终贴在窗口顶端。
- 顶部条依赖 `#app { overflow: visible }`：祖先元素一旦 `overflow: hidden`，`position: sticky` 就会失效。
- 脚注：`© 2026 法治合规知识竞赛练习 | Designed by DSH&266`。
...
- 考试模式没有倒计时（与练习模式行为一致）。
- 开始页仍有约 1460px 高（大图标 + 统计格 + 4 张模式卡）；答题页顶部占用已经从 223px 压到 97px。若还需要更短，可以再删掉 ⚖️ 大图标和描述文字。
